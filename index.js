...
const ROID_PTS_MED = 50; // points scored for a large asteroid
const ROID_PTS_SML = 100; // points scored for a large asteroid
...
var level, roids, lives, score, scoreHigh, ship, text, textAlpha,playerName = "apple";
newGame();
...
function destroyAsteroid(index) {
    ...
    if (r == Math.ceil(ROID_SIZE / 2)) { // large asteroid
        ...
        roids.push(newAsteroid(x/2, y, Math.ceil(ROID_SIZE / 8)));
        roids.push(newAsteroid(x/2, y, Math.ceil(ROID_SIZE / 8)));
        score += ROID_PTS_MED
    }else {
      score += ROID_PTS_SML
...
function newAsteroid(x, y, r) {
    ...
    var roid = {
        x: x,
        y: y,
        xv: Math.random() * ROID_SPD * lvlMultipler / FPS * (Math.random() < 0.5 ? 1 : -1),
        yv: Math.random() * ROID_SPD * lvlMultipler / FPS * (Math.random() < 0.5 ? 1 : -1),
...
ctx.fillStyle = "black";
ctx.font = "50px Impact";
ctx.fillText("Enter Your Name To Save Your High Score", canv.width/2, 100);
ctx.fillText(playerName, canv.width/2, 200);
ctx.font = "50px Impact";
